...
Prompts for the lens subagents. The main skill fills every `{...}` placeholder
before launching each worker and injects one prompt per worker.
...
sweep of the target, return your findings rows, and stop.
...
Every lens worker returns findings in this shape (see `references/ledger-format.md`
for the full field rules). Workers never assign ids or statuses; the main skill
does when merging.
