...
| 5   | Quick QR                               | 4          | medium   | M    | in progress | `0f0d00a` (WIP) | Generate/scan restyled; camera matrix OK; modules 10/20           |
...
| 2026-08-16 | 5     | **Paused mid-stage at user request (`0f0d00a`, WIP).** `AppShell`'s `stage` prop replaced with `StageSlotContext`/`useStageSlot` (a DOM node ref in context) so one screen instance can portal into both panes — needed because a screen owns its camera lifecycle and can't be two component instances. `TextEditor`, `OpticalStage/QrDisplay`, `OpticalStage/CameraScanner`, `ResultPanel`, `src/hooks/useCopy.ts` built; `QRGenerator.tsx`/`QRScanner.tsx` rewritten onto them. Verified live: QR renders via portal, camera starting/scanning chrome renders via portal, no console errors, checks green. **Remaining before Stage 5 is done**: manual camera decode + permission-denied test, screenshot modules `10`/`20` (capture.ts selectors need the same kind of update Stage 4 needed), `docs/design/flows/quick-qr.md` rewrite, legacy `.qr-*`/`.scanner*`/`.counter*` CSS removal, Standards+Spec review pass. See `docs/specs/design-system-refactor/PAUSE-2026-08-16.md` for the full resumption plan. |
